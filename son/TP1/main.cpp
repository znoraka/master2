...
static float freq = 440.0f ;		// toutes questions
...
FMOD_RESULT F_CALLBACK cb_question1 (FMOD_SOUND *sound, void *data, unsigned int datalen)
{
    ...
    {
      //      *stereo16bitbuffer++ = (signed short) (sin(M_PI * 2.0f * freq * t) * 32768.0f * ampGauche);
      //      *stereo16bitbuffer++ = (signed short) (sin(M_PI * 2.0f * freq * t) * 32768.0f * ampDroite);
            *stereo16bitbuffer++ = (signed short) (sin(M_PI * 2.0f * freq * cos(sin(tan(t))) * sin(100 / t)) * 32768.0f * ampGauche);
            *stereo16bitbuffer++ = (signed short) (sin(M_PI * 2.0f * freq * cos(sin(tan(t))) * sin(100 / t)) * 32768.0f * ampDroite);
      t += n * 1.0f/44100.0f;
    }
...
float adsr (float x)
{
  if(x < Attack)
    ...
    return Sustain * (Decay + Duree + Release - x) / Release;
  return 0 ;
}
...
int main(int argc, char *argv[])
{
  srand(time(NULL));
    ...
    if (key == '1')
    {
        ...
        mode |= FMOD_LOOP_NORMAL ;
	    createsoundexinfo.pcmreadcallback   = cb_question2;
    }
    else if (key == '6')
    {
        mode |= FMOD_LOOP_OFF ;
